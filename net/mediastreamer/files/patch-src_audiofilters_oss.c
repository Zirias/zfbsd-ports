--- src/audiofilters/oss.c.orig	2024-08-26 10:10:02 UTC
+++ src/audiofilters/oss.c
@@ -37,37 +37,36 @@ MSFilter *ms_oss_write_new(MSSndCard *card);
...
@@ -76,52 +75,22 @@ static int configure_fd(int fd, int bits, int stereo, 
...
+	ms_message("/dev/dsp opened: rate=%i,bits=%i,stereo=%i blocksize=%i.", rate, bits, stereo, blocksize);
 
 	/* start recording !!! Alex */
 	{
@@ -131,7 +100,8 @@ static int configure_fd(int fd, int bits, int stereo, 
...
@@ -151,23 +121,23 @@ typedef struct OssData {
 	bool_t stereo;
 } OssData;
...
@@ -317,59 +287,75 @@ static void *oss_thread(void *p) {
